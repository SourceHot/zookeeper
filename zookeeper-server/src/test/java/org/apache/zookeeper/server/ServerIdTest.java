...
public class ServerIdTest extends ClientBase {
    ...
    public void setUp() throws Exception {
        System.setProperty("zookeeper.extendedTypesEnabled",
                Boolean.toString(testType.ttlsEnabled));
        LOG.info("ttlsEnabled: {} - ServerId: {}", testType.ttlsEnabled, testType.serverId);
        try {
            super.setUpWithServerId(testType.serverId);
        } catch (RuntimeException e) {
            if (testType.ttlsEnabled && (testType.serverId
        ...
        try {
            ...
            if (testType.ttlsEnabled) {
                zk.create("/foo", new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE,
                        CreateMode.PERSISTENT_WITH_TTL, new Stat(), 1000);  // should work
            } else {
                try {
                    zk.create("/foo", new byte[0], ZooDefs.Ids.OPEN_ACL_UNSAFE,
                            CreateMode.PERSISTENT_WITH_TTL, new Stat(), 1000);
