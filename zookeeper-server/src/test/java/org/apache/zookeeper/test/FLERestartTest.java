...
public class FLERestartTest extends ZKTestCase {
    ...
    public void testLERestart() throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            QuorumPeer peer =
                    new QuorumPeer(peers, tmpdir[i], tmpdir[i], port[i], 3, i, 1000, 2, 2);
                    ...
                    switch (i) {
                        case 0:
                            if (peerRound == 0) {
                                ...
                                ((FastLeaderElection) restartThreads.get(
                                        i).peer.getElectionAlg()).shutdown();

                                peer = new QuorumPeer(peers, tmpdir[i], tmpdir[i], port[i], 3, i,
                                        1000, 2, 2);
                                peer.startLeaderElection();
                                peerRound++;
                            } else {
