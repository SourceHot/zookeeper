...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File snapDir,
        ...
        this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false,
                ServerCnxnFactory.createFactory(getClientAddress(quorumPeers, myid, clientPort),
                        -1),
                new QuorumMaj(quorumPeers));
    }
    ...
    private static InetSocketAddress getClientAddress(Map<Long, QuorumServer> quorumPeers,
                                                      long myid,
                                                      int clientPort)
            throws IOException {
    ...
    private void setAddrs(InetSocketAddress quorumAddr,
        ...
        synchronized (outerLockObject) {
            synchronized (QV_LOCK) {
                if (lastSeenQuorumVerifier != null
                    ...
                    LOG.error(
                            "setLastSeenQuorumVerifier called with stale config " + qv.getVersion()
                                    +
                ...
                if (writeToDisk) {
                    try {
                        ...
                        LOG.error("Error writing next dynamic config file to disk: ",
                                e.getMessage());
                    }
                }
            }
    ...
    public QuorumVerifier setQuorumVerifier(QuorumVerifier qv, boolean writeToDisk) {
        synchronized (QV_LOCK) {
            if ((quorumVerifier != null) && (quorumVerifier.getVersion() >= qv.getVersion())) {
                ...
                LOG.debug(getId() + " setQuorumVerifier called with known or old config "
                        + qv.getVersion() +
                        ". Current version: " + quorumVerifier.getVersion());
                return quorumVerifier;
            }
            ...
            quorumVerifier = qv;
            if (lastSeenQuorumVerifier == null || (qv.getVersion()
        ...
        if (prevQV.getVersion() < qv.getVersion() && !prevQV.equals(qv)) {
            ...
            if (suggestedLeaderId != null) {
                ...
                leaderChange = (myleaderInCurQV == null || myleaderInCurQV.addr == null ||
                        myleaderInNewQV == null || !myleaderInCurQV.addr.equals(
                        myleaderInNewQV.addr));
    ...
    public static final class AddressTuple {
        ...
        public AddressTuple(InetSocketAddress quorumAddr,
                            InetSocketAddress electionAddr,
                            InetSocketAddress clientAddr) {
    ...
    public static class QuorumServer {
        ...
        // VisibleForTesting
        public QuorumServer(long id, InetSocketAddress addr) {
            this(id, addr, (InetSocketAddress) null, (InetSocketAddress) null,
        ...
        public QuorumServer(long sid, String addressStr) throws ConfigException {
            ...
            if (serverClientParts.length == 2) {
                ...
                try {
                    ...
                    throw new ConfigException(
                            "Address unresolved: " + hostname + ":" + clientParts[clientParts.length
                                    - 1]);
                }
            }
            ...
            try {
                ...
                throw new ConfigException(
                        "Address unresolved: " + serverParts[0] + ":" + serverParts[1]);
            }
            try {
                ...
                throw new ConfigException(
                        "Address unresolved: " + serverParts[0] + ":" + serverParts[2]);
            }
        ...
        public void checkAddressDuplicate(QuorumServer s) throws BadArgumentsException {
            ...
            for (InetSocketAddress my : this.myAddrs) {
                ...
                for (InetSocketAddress other : otherAddrs) {
                    if (my.equals(other)) {
                        String error =
                                String.format("%s of server.%d conflicts %s of server.%d", my,
                                        this.id, other, s.id);
